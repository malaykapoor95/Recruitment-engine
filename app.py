...
# --- 1. BRANDING (Top Right Header ONLY) ---
...
# --- 3. MAIN PAGE ACCESS GATE ---
# We check if the user has already entered a valid code in this session
if 'access_granted' not in st.session_state:
    st.session_state.access_granted = False

if not st.session_state.access_granted:
    # This replaces the large logo on the landing page
    st.info("Welcome to Trident Project Command.")
    user_code = st.text_input("Please enter your Center Access Code to begin:", placeholder="e.g. TR-C1")
    ...
    if user_code:
        if user_code in ACCESS_CODES:
            ...
            st.error("Invalid Access Code. Please try again or contact the administrator.")
    
    # Explicitly stopping the script here so nothing else renders until they log in
    st.stop()
