...
with col_logo:
    # This is the only logo call in the entire script
...
# Visible text input (type="password" removed)
    ...
    if role == "Recruitment":
        ...
        if st.session_state.step == 1:
            ...
            for i in range(num_pax):
                ...
                rid = c_a.text_input(f"Respondent ID", key=f"rid_{i}")
                fn = c_b.text_input(f"First Name", key=f"fn_{i}")
                status = c_c.selectbox("Status", ["Fresh", "Repeat"], key=f"stat_{i}")
                gen = c_d.selectbox("Gender", ["Male", "Female"], key=f"g_{i}")
                c_e, c_f, c_g = st.columns(3)
                race = c_e.selectbox("Race", ["East Asian", "South Asian", "Black", "Hispanic", "White", "Middle East", "Native American"], key=f"r_{i}")
                h = c_f.number_input("Height (In)", 58, 85, key=f"h_{i}")
                age = c_g.selectbox("Age Group", ["20-30", "30-40", "40-50", "50-60"], key=f"a_{i}")
            ...
            if cc.button("CONFIRM & SYNC", type="primary"):
